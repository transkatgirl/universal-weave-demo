...
pub struct TreeView<'a> {
    ...
    /// Fraction of the viewport's height that is visible, cropped evenly
    /// around the center. Below 1.0 when a panel borrows rows from the tree;
    /// keeps the drawing scale constant instead of squeezing the graph.
    pub vertical_crop: f64,
}
...
{
    let mut layouter = TopologicalLayouter::<u64, DefaultHashBuilder>::new(Spacing {
        node: 6.0,
        layer: 12.0,
        corridor: 0.0,
        edge: 4.0,
    });
    <TopologicalLayouter<u64, DefaultHashBuilder> as Layouter<
        W,
        u64,
        N,
        T,
        Vec2,
        ArrayVec<[Vec2; 6]>,
    >>::layout(&mut layouter, weave, |_| Vec2::new(NODE_HEIGHT, NODE_WIDTH));

    let computed_size = <TopologicalLayouter<u64, DefaultHashBuilder> as Layouter<
        W,
        u64,
        N,
        T,
...
pub fn render(frame: &mut Frame, area: Rect, view: TreeView<'_>, hint: &str) {
    let TreeView {
        nodes,
        layout,
        viewport,
    ...
    if nodes.is_empty() {
        frame.render_widget(
            Paragraph::new("The weave is empty. Add a root node from the menu.")
    ...
    let y_bounds = crop_bounds(y_bounds, vertical_crop);
...
/// Shrink a coordinate range to `fraction` of its height around its midpoint.
fn crop_bounds(bounds: [f64; 2], fraction: f64) -> [f64; 2] {
    let mid = (bounds[0] + bounds[1]) / 2.0;
    let half = (bounds[1] - bounds[0]) * fraction.clamp(0.0, 1.0) / 2.0;
    [mid - half, mid + half]
}
...
#[cfg(test)]
mod tests {
    use super::super::content::{IndependentDemoNode, IndependentDemoWeave, TextContent};
    use super::*;
    ...
    #[test]
    fn crop_bounds_shrinks_evenly_around_the_midpoint() {
        assert_eq!(crop_bounds([0.0, 40.0], 0.5), [10.0, 30.0]);
        assert_eq!(crop_bounds([10.0, 30.0], 1.0), [10.0, 30.0]);
        // Fractions above 1.0 never zoom the view out.
        assert_eq!(crop_bounds([0.0, 40.0], 1.5), [0.0, 40.0]);
    }
