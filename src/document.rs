...
use crate::content::{DemoNode, DemoWeave, IndependentDemoNode, IndependentDemoWeave, TextContent};
...
/// A document wraps one of the two weave implementations, plus its `LoggedWeave` action log.
///
...
pub fn seeded_dependent() -> Document {
    ...
    weave.add_node(node(0, None, false, false, "The lighthouse keeper found the letter on a Tuesday. "));
    weave.add_node(node(1, Some(0), false, true, "It was written in a language that smelled of salt. "));
    weave.add_node(node(3, Some(1), true, false, "She read it three times before the lamp went out. "));
    weave.add_node(node(2, Some(0), false, false, "It was addressed to someone who had drowned fifty years ago. "));
    weave.add_node(node(4, Some(2), false, false, "That night, the sea began to knock."));
